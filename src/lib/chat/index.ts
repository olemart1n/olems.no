...
export const scrollToBottom = (messageEndRef: Signal<HTMLDivElement | undefined>) => {
    setTimeout(() => {
      messageEndRef.value?.scrollIntoView({ behavior: "smooth", block:"end" });
    }, 10);
}

export const setSocketEventListeners = (conn: WebSocket, visitorCount: Signal<number>, messages: MessageBroadcast [], messageEndRef: Signal<HTMLDivElement | undefined>) => {
...
export const setInputFormSubmitEvent = (inputForm: Signal, conn: WebSocket, username: Signal, messages: MessageBroadcast [], messageEndRef: Signal<HTMLDivElement | undefined>) => {
